...
const TransactionBarChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getTopClientsTransactions(5);
        if (res.clients) {
          setData(res.clients.map(c => ({
            name: c.name.split(' ')[0],
    ...
    <div className="chart-container" style={{ width: '100%', height: '300px' }}>
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 20, right: 20, left: 20, bottom: 5 }}>
          ...
          <YAxis yAxisId="left" orientation="left" stroke="#8884d8" />
          <YAxis yAxisId="right" orientation="right" stroke="#82ca9d" />
          <Tooltip formatter={(value, name) => name === 'purchaseAmount' ? [`€${value.toFixed(2)}`, 'Amount'] : [value, 'Count']} />
          <Legend />
          <Bar yAxisId="left" dataKey="purchaseAmount" name="Purchase Amount (€)" fill="#8884d8" />
          <Bar yAxisId="right" dataKey="purchaseCount" name="Purchase Count" fill="#82ca9d" />
        </BarChart>
